...
export default function WorkflowSection() {
  return (
    <section id="workflow" className="relative bg-bg1 text-bg0 py-24 px-6 z-30 border-t border-bg0/10">
      <div className="max-w-4xl mx-auto flex flex-col items-center">
        {/* Section Divider Line */}
        <div className="w-full h-[1px] bg-bg0/20 mb-8" />

        {/* Top Half: How It Works */}
        <h2 className="text-3xl md:text-5xl font-extrabold text-bg0 text-center leading-tight tracking-tight max-w-2xl mx-auto py-2">
          How It Works
        </h2>

        {/* Section Divider Line */}
        <div className="w-full h-[1px] bg-bg0/20 mt-8 mb-16" />
        ...
        <div className="w-full relative max-w-3xl mx-auto">
          ...
          <div className="hidden md:block absolute top-1/2 left-0 right-0 h-1 bg-bg0/20 -translate-y-1/2 z-0">
            <div className="h-full bg-gradient-to-r from-bg0 via-emerald-400 to-bg0 w-full animate-pulse opacity-80" />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 relative z-10">
            {workflowSteps.map((item, idx) => (
              <div
                key={idx}
                className="group bg-bg0/10 hover:bg-bg0/20 border border-bg0/20 p-6 flex flex-col items-center text-center transition-all duration-300 hover:-translate-y-1.5 backdrop-blur-md relative overflow-hidden"
              >
                {/* Step Badge */}
                ...
                {idx < workflowSteps.length - 1 && (
                  <div className="md:hidden mt-4 text-bg0/40 animate-bounce">
                    ↓
                  </div>
                )}
              </div>
        ...
        <div className="w-full mt-28 pt-16 border-t border-bg0/20 flex flex-col items-center">
          <h3 className="text-3xl md:text-4xl font-extrabold text-bg0 text-center tracking-tight mb-3">
            Built For The Future.
          </h3>
          <p className="text-base md:text-lg text-bg0/80 font-medium text-center mb-12 max-w-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-3xl">
            {futureFeatures.map((item, idx) => (
              <div
                key={idx}
                className="group bg-bg0/10 hover:bg-bg0/15 border border-bg0/20 backdrop-blur-md p-6 flex items-center justify-between shadow-lg transition-all duration-300 hover:-translate-y-1"
              >
